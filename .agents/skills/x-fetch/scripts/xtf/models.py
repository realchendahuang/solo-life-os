...
class Tweet:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # local patch (X-Tiller): Nitter/snapshot cards carry no view count, so
        # a 0 here means "unknown", not "measured 0".
        if self.views == 0:
            d["views_unknown"] = True
        if self.media:
            d["media"] = self.media
...
class Reply:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # local patch (X-Tiller): Nitter/snapshot replies carry no view count.
        if self.views == 0:
            d["views_unknown"] = True
        if self.retweets:
